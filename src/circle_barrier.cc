...
CyclicBarrier::CyclicBarrier(const unsigned int
...
unsigned int CyclicBarrier::dowait(const bool

                                   &timeOutNeeded, const long &waitTime, const TimeUnit &unit) {
...
// We are here means we are not the last or tripping thread.
    if (!timeOutNeeded)
        m_cond.
                wait(exclusiveLock,
                     [&]() {
                         return (this->m_count == m_parties || this->m_broken == true);
                     });
    else
        m_cond.
                wait_for(exclusiveLock, TimeUtils::waitDuration(waitTime, unit),
                         [&]() {
                             return (this->m_count == m_parties || this->m_broken == true);
                         });
...
unsigned int CyclicBarrier::await(const long

                                  &waitTimeInMilliSecs, const TimeUnit &unit) {
    return (
            dowait(true, waitTimeInMilliSecs, unit
            ));
}
