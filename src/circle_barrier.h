...
#define CDCF_PERFORMANCE_MEASURING_CIRCLE_BARRIER_H
...
private:
    ...
    void
    nextGeneration();                    // This creates the next cycle/generation of the Barrier upon successfull completion or upon reset.
    void breakBarrier();                    // This is used to break the barrier and wakeup or signal everyone.
    unsigned int dowait(const bool &, const long &,
                        const TimeUnit &);    // This returns the arrival index upon succes else throws exception due to BarrierBroken or TimeOut.
...
public:
    ...
    unsigned int await(const long &,
                       const TimeUnit & = TimeUnit::MilliSeconds);        // Ths is  await with time out feature. It either returns suceessfull or breaks the barrier upon timeout.
    ...
    unsigned int
    getNumberWaiting();        // This returns the number of parties that are currently waiting at the barrier. Done with a Lock.
